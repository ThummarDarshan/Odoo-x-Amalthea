...
import { Button, Container, Form, Modal, Table, Badge } from "react-bootstrap";
...
const TableData = (props) => {
  ...
  const getStatusBadge = (item) => {
    // For expenses, check the status field
    if (item.transactionType === 'expense' && item.status) {
      const statusConfig = {
        'pending_approval': { variant: 'warning', text: 'Pending' },
        'approved': { variant: 'success', text: 'Accepted' },
        'rejected': { variant: 'danger', text: 'Rejected' },
        'submitted': { variant: 'info', text: 'Submitted' },
        'draft': { variant: 'secondary', text: 'Draft' }
      };
      
      const config = statusConfig[item.status] || { variant: 'secondary', text: item.status };
      return <Badge bg={config.variant}>{config.text}</Badge>;
    }
    
    // For credits/other transactions, show as approved
